            ...
            for rdn in rdn_seq:
                for name in rdn:
                    if name['type'].dotted == cfg_opt[ce].oid:
                        found = True
                        break
            output(found, ce, cfg_opt, cfg_sect, str(name.native), outJson)
...
def lint_key_usage(cfg_opt, cert, cfg_sect, outJson):
    ...
    if found:
        ...
        output(e['extn_value'].native == 'digital_signature', "key_usage_digitalSignature", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == 'non_repudiation', "key_usage_nonRepudiation", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == 'key_encipherment', "key_usage_keyEncipherment", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == 'data_encipherment', "key_usage_dataEncipherment", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == 'key_agreement', "key_usage_keyAgreement", cfg_opt, cfg_sect,e['extn_value'].native, outJson)
        output(e['extn_value'].native == 'key_cert_sign', "key_usage_keyCertSign", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == 'crl_sign', "key_usage_cRLSign", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == 'encipher_only', "key_usage_encipherOnly", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == 'decipher_only', "key_usage_decipherOnly", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
    else:
            ...
            for rdn in rdn_seq:
                for name in rdn:
                    if name['type'].dotted == cfg_opt[ce].oid:
                        found = True
                        break
            output(found, ce, cfg_opt, cfg_sect, str(rdn), outJson)
        elif "subject_base_dn" in ce:
...
def lint_skid(cfg_opt, cert, cfg_sect, outJson):
    ...
    output(found, "skid_present", cfg_opt, cfg_sect, e['extn_id'], outJson)
    if found:
...
def lint_serial_number(cfg_opt, cert, cfg_sect, outJson):
    ...
    output(ln > int(cfg_opt["serial_min_length"].value),  "serial_min_length", cfg_opt, cfg_sect, cert_leaf, outJson)
    output(ln < int(cfg_opt["serial_max_length"].value), "serial_max_length", cfg_opt, cfg_sect, cert_leaf, outJson)
    return
...
def lint_cert_policies(cfg_opt, cert, cfg_sect, outJson):
    ...
    for ce in cfg_opt:
        if ce == "certificate_policies_present":
            output(not c_policies == None, ce, cfg_opt, cfg_sect, str(e), outJson)
    ...
    if found:
        ...
        for item in e['extn_value'].native:
            if item['access_method'] == 'ca_issuers':
                ...
                ocsp_present = True
        output(ca_issuers_present, "aia_ca_issuers_present", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(ocsp_present, "aia_ocsp_present", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
...
def lint_eku(cfg_opt, cert, cfg_sect, outJson):
    ...
    if found:
        ...
        output(e['extn_value'].native == cfg_opt['eku_oid_server_auth'].oid, "eku_oid_server_auth", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_client_auth'].oid, "eku_oid_client_auth", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_code_signing'].oid, "eku_oid_code_signing", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_email_protection'].oid, "eku_oid_email_protection", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_time_stamping'].oid, "eku_oid_time_stamping", cfg_opt, cfg_sect,e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_ocsp_signing'].oid, "eku_oid_ocsp_signing", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_any_eku'].oid, "eku_oid_any_eku", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_smart_card_logon'].oid, "eku_oid_smart_card_logon", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_ipsec_ike_intermediate'].oid, "eku_oid_ipsec_ike_intermediate", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_ipsec_end_system'].oid, "eku_oid_ipsec_end_system", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_ipsec_tunnel_termination'].oid, "eku_oid_ipsec_tunnel_termination", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_ipsec_user'].oid, "eku_oid_ipsec_user", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_piv_card_auth'].oid, "eku_oid_piv_card_auth", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_pivi_content_signing'].oid, "eku_oid_pivi_content_signing", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_smartCardLogon'].oid, "eku_oid_smartCardLogon", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_pkinit_KPKdc'].oid, "eku_oid_pkinit_KPKdc", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_oid_pkinit_KPClientAuth'].oid, "eku_oid_pkinit_KPClientAuth", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(e['extn_value'].native == cfg_opt['eku_other'].oid, "eku_other", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
    ...
    if found:
        ...
        for item in e['extn_value'].native:
            if "http" in item['distribution_point']:
                ...
                crldp_ldap = True
        output(crldp_http, "crldp_http", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
        output(crldp_ldap, "crldp_ldap", cfg_opt, cfg_sect, e['extn_value'].native, outJson)
...
def lint_signature_algorithm(cfg_opt, cert, cfg_sect, outJson):
    ...
    output(found and cert_leaf.dotted == cert_leaf1, ce, cfg_opt, cfg_sect, str(cert_leaf.dotted), outJson)
    return
...
def output(result, ce, cfg_opt, cfg_sect, cert_value, outJson):
    if result is True and cfg_opt[ce].value is '1' or result is False and cfg_opt[ce].value is '2':
      outJson.append("{'Section': '" + cfg_sect + "'," +
                      "'Item': '" + ce + "'," +
